...
class ProxyEndpoint {
   ...
   * @param  {Array<Rule>} proxyReqRules Any proxy request rules
   * @param  {Array<Rule>} proxyResRules Any proxy result rules
   */
  constructor ({ source, target = {} }, middlewareRules = [], proxyReqRules = [], proxyResRules = []) {
    this.source = source;
    this.target = target;
